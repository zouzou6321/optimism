...
// TestChannelBankInterleavedPreMeadow ensure that the channel bank can handle frames from multiple channels
// that arrive out of order. Per the specs, the first channel to arrive (not the first to be completed)
// is returned first prior to the Meadow network upgrade
func TestChannelBankInterleavedPreMeadow(t *testing.T) {
	rng := rand.New(rand.NewSource(1234))
	a := testutils.RandomBlockRef(rng)
	...
	cfg := &rollup.Config{ChannelTimeout: 10, MeadowTime: nil}
...
// that arrive out of order. Per the specs (post meadow), the first channel to be complete should be
// returned
func TestChannelBankInterleaved(t *testing.T) {
	...
	mt := uint64(0)
	cfg := &rollup.Config{ChannelTimeout: 10, MeadowTime: &mt}
