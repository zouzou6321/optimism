...
func (cb *ChannelBank) Read() (data []byte, err error) {
	...
	// out channels will eventually be removed.
	if len(cb.channelQueue) == 0 {
		return nil, io.EOF
	}
	...
	// Pre-Meadow we simply check the first index.
	// Post-Meadow we read the entire channelQueue for the first ready channel. If no channel is
	// available, we return `nil, io.EOF`.
	// TODO: Activation time should be more precise
	if !cb.cfg.IsMeadow(cb.Origin().Time) {
...
func (cb *ChannelBank) readIndex(i int) (data []byte, err error) {
	...
	// TODO: What do the metrics mean now?
	if len(cb.channelQueue) > 0 {
		cb.metrics.RecordHeadChannelOpened()
	}
	r := ch.Reader()
