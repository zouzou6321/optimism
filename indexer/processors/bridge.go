...
func (b *BridgeProcessor) process() error {
	...
	if latestEpoch == nil {
		...
		if b.LatestL1Header != nil || b.LatestL2Header != nil {
			b.log.Error("bridge events indexed, but no latest epoch found", "latest_bridge_l1_block_number", b.LatestL1Header.Number)
			return errors.New("bridge events indexed, but no indexed epoch returned")
		}

		b.log.Warn("no indexed epochs available. waiting...")
		return nil
	} else if b.LatestL1Header != nil && latestEpoch.L1BlockHeader.Hash == b.LatestL1Header.Hash() {
